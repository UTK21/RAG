...
# ---------------- Header ----------------
ax.text(0.5, 7.15, "BUILDING RAG FROM SCRATCH  ·  PART 5", ha="left", va="center",
        fontsize=10.5, fontweight="bold", color=CYAN, fontfamily="monospace")
ax.text(6.5, 6.6, "Parent-Child Chunking — Small for Matching, Big for Context",
        ha="center", va="center", fontsize=18, fontweight="bold", color=WHITE)
ax.text(6.5, 6.15, "Embed small children  →  match  →  fetch big parent  →  LLM",
        ha="center", va="center", fontsize=12, color=GRAY, fontfamily="monospace")

ax.plot([0.5, 12.5], [5.72, 5.72], color=LINE, linewidth=1.2)

# ---------------- Row 1: Before ----------------
ax.text(0.5, 5.42, "THE CONFLICT  ·  ONE CHUNK SIZE, TWO JOBS", ha="left", va="center",
...
row1 = [
    ("ONE CHUNK SIZE", "must serve 2 jobs",                ROSE, False),
    ("RETRIEVAL WANTS", "small · one idea each",           ROSE, False),
    ("LLM WANTS",        "large · full context",            ROSE, False),
    ("✗ COMPROMISE",     "mushy vectors OR missing context",ROSE, True),
]
xs1 = []
...
# ---------------- Row 2: After ----------------
ax.text(0.5, 2.50, "THE FIX  ·  PARENT-CHILD CHUNKING", ha="left", va="center",
...
row2 = [
    ("PARENT + CHILDREN", "1200w parent · 3×240w children", AMBER,  False),
    ("EMBED CHILDREN",    "FAISS + BM25",                   PURPLE, False),
    ("BEST CHILD MATCHES","precise, focused vector",        CYAN,   False),
    ("FETCH PARENT",      "via parent_idx (dedupe)",        AMBER,  False),
    ("✓ LLM CONTEXT",     "precise match + rich context",   GREEN,  True),
]
xs2 = []
...
# Connector: ONE CHUNK SIZE (before) -> PARENT + CHILDREN (after) — the split happens here
arrow(xs1[0] + w1 / 2, y1, xs2[0] + w2 / 2, y2 + h2, color=WHITE, lw=1.8, ls=(0, (4, 3)))
mid_x = (xs1[0] + w1 / 2 + xs2[0] + w2 / 2) / 2
mid_y = (y1 + (y2 + h2)) / 2
ax.text(mid_x + 0.1, mid_y, "  same data,\n  now two sizes", ha="left", va="center",
        fontsize=8.5, color=WHITE, fontfamily="monospace")

# ---------------- Footer ----------------
ax.text(6.5, 0.42,
        'retrieval granularity ≠ generation granularity — a parent_idx pointer links them',
        ha="center", va="center", fontsize=10.5, color=GRAY,
        fontfamily="monospace", style="italic")
ax.text(12.5, 0.42, "05 / 06", ha="right", va="center",
        fontsize=10, color=GRAY, fontfamily="monospace")

plt.tight_layout()
plt.savefig("/home/user/RAG/parent_child_pipeline_dark.png", dpi=150, bbox_inches="tight",
            facecolor=BG)
print("saved")
