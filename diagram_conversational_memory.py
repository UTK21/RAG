...
# ---------------- Header ----------------
ax.text(0.5, 7.15, "BUILDING RAG FROM SCRATCH  ·  PART 2", ha="left", va="center",
        fontsize=10.5, fontweight="bold", color=CYAN, fontfamily="monospace")
ax.text(6.5, 6.6, "Conversational Memory — Making Follow-ups Work", ha="center", va="center",
        fontsize=20, fontweight="bold", color=WHITE)
ax.text(6.5, 6.15, "History + Question  →  Rewrite (LLM)  →  Standalone Query  →  Retrieval",
        ha="center", va="center", fontsize=12, color=GRAY, fontfamily="monospace")

ax.plot([0.5, 12.5], [5.72, 5.72], color=LINE, linewidth=1.2)

# ---------------- Row 1: Before ----------------
ax.text(0.5, 5.42, "BEFORE  ·  NAIVE RETRIEVAL", ha="left", va="center",
...
row1 = [
    ("FOLLOW-UP Q", "no context", ROSE, False),
    ("EMBED", "literal words only", ROSE, False),
    ("SEARCH", "FAISS top-k", ROSE, False),
    ("✗ WRONG CHUNKS", "garbage results", ROSE, True),
]
xs1 = []
...
# ---------------- Row 2: After ----------------
ax.text(0.5, 2.50, "AFTER  ·  WITH QUERY REWRITING", ha="left", va="center",
...
row2 = [
    ("HISTORY + Q", "last N turns", CYAN, False),
    ("REWRITE", "LLM · temp=0", PURPLE, False),
    ("STANDALONE Q", "context resolved", AMBER, False),
    ("SEARCH", "FAISS top-k", AMBER, False),
    ("✓ RIGHT CHUNKS", "correct context", GREEN, True),
]
xs2 = []
...
# Connector: EMBED (before) -> REWRITE (after) — the one step that gets inserted
arrow(xs1[1] + w1 / 2, y1, xs2[1] + w2 / 2, y2 + h2, color=WHITE, lw=1.8, ls=(0, (4, 3)))
mid_x = (xs1[1] + w1 / 2 + xs2[1] + w2 / 2) / 2
mid_y = (y1 + (y2 + h2)) / 2
ax.text(mid_x + 0.05, mid_y, "  one step\n  inserted here", ha="left", va="center",
        fontsize=8.5, color=WHITE, fontfamily="monospace")

# ---------------- Footer ----------------
ax.text(6.5, 0.42,
        'retrieval uses the REWRITTEN query · generation still uses the ORIGINAL question + history',
        ha="center", va="center", fontsize=10.5, color=GRAY,
        fontfamily="monospace", style="italic")
ax.text(12.5, 0.42, "02 / 06", ha="right", va="center",
        fontsize=10, color=GRAY, fontfamily="monospace")

plt.tight_layout()
plt.savefig("/home/user/RAG/conversational_memory_pipeline_dark.png", dpi=150, bbox_inches="tight",
            facecolor=BG)
print("saved")
